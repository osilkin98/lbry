...
class UnAuthAPIClient:
    ...
    async def call(self, method, params=None):
        ...
        async with self.session.get('http{}://{}:{}'.format("" if not conf.settings['use_https'] else "s",
                                                            self.host, self.port), json=message) as resp:
...
class AuthAPIClient:
    ...
    async def get_client(cls, key_name=None):
        ...
        login_url = "http{}://{}:{}@{}:{}".format("" if not conf.settings['use_https'] else "s", api_key_name,
                                                  api_key.secret, conf.settings['api_host'], conf.settings['api_port'])
